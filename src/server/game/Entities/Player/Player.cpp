...
#endif
Player::Player (WorldSession *session): Unit(this), _achievementMgr(this), _reputationMgr(this), m_MaxPlayerbots(9), m_bot_died(false)
 {
    // Playerbot mod
    m_playerbotAI = NULL;
...
void Player::DeleteFromDB(uint64 playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    // we set it to mode CHARACTER_DELETE_REMOVE
    ...
    switch (charDelete_method)
    {
        ...
        {
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHARACTER_UPDATE_DELETE_INFO);
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        if (q_status.Status == QUEST_STATUS_INCOMPLETE && (!GetGroup() || !GetGroup()->isRaidGroup() || qInfo->IsAllowedInRaid()))
        {
            if (qInfo->HasFlag(QUEST_SKYFIRE_FLAGS_PLAYER_KILL))
            {
				...
				//Playerbot mod
				if(m_playerbotAI != NULL) m_playerbotAI->KilledMonster(qInfo->GetQuestId(), m_playerbotAI->GetPlayerBot()->GetGUID());
...
bool Player::HasItemFitToSpellRequirements(SpellEntry const* spellInfo, Item const* ignoreItem)
{
    ...
    // scan other equipped items for same requirements (mostly 2 daggers/etc)
    // for optimize check 2 used cases only
    switch (test->EquippedItemClass)
    {
        case ITEM_CLASS_WEAPON:
        {
            for (uint8 i= EQUIPMENT_SLOT_MAINHAND; i < EQUIPMENT_SLOT_TABARD; ++i)
                if (Item *item = GetUseableItemByPos(INVENTORY_SLOT_BAG_0, i))
                    if (item != ignoreItem && item->IsFitToSpellRequirements(test))
                        return true;
            break;
        }
        case ITEM_CLASS_ARMOR:
