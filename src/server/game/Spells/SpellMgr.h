...
enum SpellFamilyFlag
{
    ...
    SPELLFAMILYFLAG_ROGUE_VAN_SPRINT        = 0x00000840,   // Vanish, Sprint
...
inline uint32 GetAllSpellMechanicMask(SpellInfo const* spellInfo)
{
	SpellEffectInfo test;
    uint32 mask = 0;
    if (spellInfo->Mechanic)
        mask |= 1<<spellInfo->Mechanic;
    for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
		if (spellInfo->Effects[i].Effect && spellInfo->Effects[i].Mechanic)
            mask |= 1<<spellInfo->Effects[i].Mechanic;
    return mask;
}
...
uint32 GetSpellDuration(SpellInfo const *spellInfo);

#define sSpellMgr ACE_Singleton<SpellMgr, ACE_Null_Mutex>::instance()

inline SpellSchoolMask GetSpellSchoolMask(SpellEntry const* spellInfo){return SpellSchoolMask(spellInfo->SchoolMask);}
