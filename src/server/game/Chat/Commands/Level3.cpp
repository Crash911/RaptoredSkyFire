...
bool ChatHandler::HandleQuestAdd(const char *args)
{
    ...
    if (!player)
    {
        SendSysMessage("Fehler");//LANG_NO_CHAR_SELECTED
        SetSentErrorMessage(true);
        return false;
    }
    ...
    if (!pQuest)
    {
        PSendSysMessage(LANGUAGE_COMMAND_QUEST_NOTFOUND,entry);
        SetSentErrorMessage(true);
        return false;
    }

    // check item starting quest (it can work incorrectly if added without item in inventory)
	for (uint32 id = 0; id < sItemStore.GetFieldCount(); id++)
    {
        ItemEntry const *pProto = sItemStore.LookupEntry(id);
		ItemTemplate *test;
		test->ItemId = pProto->ID;
        if (!pProto)
            continue;

		if (test->StartQuest == entry)
        {
			PSendSysMessage(LANGUAGE_COMMAND_QUEST_STARTFROMITEM, entry, pProto->ID);
            SetSentErrorMessage(true);
            return false;
        }
    }
...
bool ChatHandler::HandleQuestRemove(const char *args)
{
   ...
   // player->getQuestStatusMap()[entry].m_rewarded = false;
    ...
    SendSysMessage(LANGUAGE_COMMAND_QUEST_REMOVED);
    return true;
}
...
bool ChatHandler::HandleQuestComplete(const char *args)
{
    ...
    if (!player)
    {
        SendSysMessage("Fehler");//LANGUAGE_NO_CHAR_SELECTED
        SetSentErrorMessage(true);
        return false;
    }
