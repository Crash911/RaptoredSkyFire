...
bool PlayerbotClassAI::listAuras(Unit *u)
{
    ...
    for(Unit::AuraMap::const_iterator itr = vAuras.begin(); itr!=vAuras.end(); ++itr)
    {
        ...
        const SpellInfo *spellInfo = itr->second->GetSpellInfo();//char to string
        const std::string name = chr2str(spellInfo->SpellName[loc]);
...
bool PlayerbotClassAI::castSelfCCBreakers (uint32 castList[])
{
    ...
    /*  if( GetPlayerBot()->getRace() == RACE_BLOODELF && !pTarget->HasAura( ARCANE_TORRENT,0 ) && castSpell( ARCANE_TORRENT,pTarget ) ) {
         //GetPlayerBot()->Say("Arcane Torrent!", LANGUAGE_UNIVERSAL);
        ...
        //GetPlayerBot()->Say("EVERY MAN FOR HIMSELF!", LANGUAGE_UNIVERSAL);
       ...
       // GetPlayerBot()->Say("WILL OF THE FORSAKEN!", LANGUAGE_UNIVERSAL);
        ...
        //GetPlayerBot()->Say("STONEFORM!", LANGUAGE_UNIVERSAL);
       ...
       // GetPlayerBot()->Say("ESCAPE ARTIST!", LANGUAGE_UNIVERSAL);
    } */
...
{
    Unit *pUnit = NULL;
    SkyFire::MostHPMissingInRange u_check(GetPlayerBot(), range, MinHPDiff);
    SkyFire::UnitLastSearcher<SkyFire::MostHPMissingInRange> searcher(GetPlayerBot(), pUnit, u_check);
