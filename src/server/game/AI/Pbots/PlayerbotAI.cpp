...
std::string chr2str2(char input)
{
	std::stringstream ss;
	std::string ret;
	ss << input;
...
m_FeastSpamTimer(0)
{
    ...
    if(master->GetGroup())
    {
            ...
            m_group->IsMember(bot->GetGUID()) )
        {
            ...
            bot->Say("Group is full!", LANGUAGE_UNIVERSAL);
        }
    } else {
        ...
        case CMSG_MOVE_SPLINE_DONE:
        {
            ...
            for(PlayerBotMap::const_iterator it = masterSession.GetPlayerBotsBegin(); it != masterSession.GetPlayerBotsEnd(); ++it)
            {
                ...
                if(!bot->IsInMap((WorldObject*) masterSession.GetPlayer()))
                {
                    ...
                    uint32 sourcenode = bot->_taxi.GetTaxiSource();
...
// sLog->outError ("PlayerbotAI::UseMount: %s is mounted but %s is not", m_master->GetName(), m_bot->GetName());
        ...
        {
            master_speed1 = m_master->GetAuraEffectsByType(SPELL_AURA_MOUNTED).front()->GetSpellInfo()->Effects->BasePoints;
			master_speed2 = m_master->GetAuraEffectsByType(SPELL_AURA_MOUNTED).front()->GetSpellInfo()->Effects->BasePoints;
        }
...
//else sLog->outError ("%s is attacking %s", m_bot->GetName(), thingToAttack->GetName());
    ...
    {
        Unit *pUnit = NULL;
        SkyFire::NearestHostileUnitInAttackDistanceCheck u_check((Creature*)m_bot, 30.0);
        SkyFire::UnitLastSearcher<SkyFire::NearestHostileUnitInAttackDistanceCheck> searcher(m_bot, pUnit, u_check);
    ...
    if(thingToAttack->GetTypeId() != TYPEID_PLAYER)
    {
        ...
        CreatureTemplate const *cInfo = ((Creature *)thingToAttack)->GetCreatureTemplate();
...
void PlayerbotAI::SendWhisper(const std::string &text, Player &player)
{
    ...
    m_bot->BuildPlayerChat(&data, CHAT_MSG_WHISPER_INFORM, text, LANGUAGE_UNIVERSAL);
...
bool PlayerbotAI::CastSpell(const SpellInfo * pSpellInfo, Unit *target, bool checkFirst, bool castExistingAura, bool skipFriendlyCheck, bool skipEquipStanceCheck, bool triggered)
{
    ...
    {
        //NEGATIVE SPELL
		if (sSpellMgr->GetSpellInfo(spellId) && SPELL_ATTR0_CU_NEGATIVE)
        {
...
bool PlayerbotAI::CastSpell(const SpellEntry * pSpellInfo, Unit *target, bool checkFirst, bool castExistingAura, bool skipFriendlyCheck, bool skipEquipStanceCheck, bool triggered)
{
    ...
    if ( m_bot->GetSelection() != target->GetGUID() ) { m_bot->SetSelection(target->GetGUID()); } //if target is different than selection apply it
	SpellInfo *test = 0;
	test->Id = pSpellInfo->Id;
    m_bot->CastSpell(target, test, triggered); //CAST THE SPELL
    if ( m_bot->GetSelection() != oldSel ) { m_bot->SetSelection(oldSel); } // Restore if target changed to cast
...
bool PlayerbotAI::CanCast(const SpellEntry * pSpellInfo, Unit *target, bool castExistingAura, bool skipFriendlyCheck, bool skipEquipStanceCheck)
{
	...
	uint32 pPowerCost = info->CalcPowerCost(m_bot, pSpellSchool);
    ...
    if (info->CastTimeEntry->CastTime > 0 && m_bot->isMoving()) return false; //Cannot cast while moving

    //The target is immune or not?
    if (target->GetTypeId() != TYPEID_PLAYER) { if ( ((Creature*)target)->IsImmunedToSpell(info)) { return false; } }
    else { if (target->IsImmunedToSpell(info)) { return false; } }
...
bool PlayerbotAI::CanCast(const SpellInfo * pSpellInfo, Unit *target, bool castExistingAura, bool skipFriendlyCheck, bool skipEquipStanceCheck)
{
    ...
    else if (pSpellInfo->_IsPositiveSpell())
    {
        if(! m_bot->IsFriendlyTo(target)) { return false; }
        else if (pSpellRange->maxRangeFriend != 0) { if (pSpellRange->maxRangeFriend < curDistance || pSpellRange->minRangeFriend > curDistance) { return false; } }
        else if (curDistance > MELEE_RANGE) return false;
...
void PlayerbotAI::UseItem(Item &item)
{
    ...
    uint32 target = TARGET_FLAG_NONE;
...
void PlayerbotAI::HandleCommand(const std::string &text, Player &fromPlayer)
{
    ...
    {
        m_bot->Say ("Thank you for this treat", LANGUAGE_UNIVERSAL);
    ...
    else if(text == "spells")
    {
        ...
        for(PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
        {
            ...
            std::string comp = ",";
            comp.append(chr2str2(pSpellInfo->SpellName[loc]));
            comp.append(",");
 ...
 else if (text.size() > 2 && text.substr(0, 2) == "g " || text.size() > 4 && text.substr(0, 4) == "get ")
 {
     ...
     if (extractGOinfo(text, guid, entry, mapid, x, y, z))
     {
         ...
         if (!go)
         {
             m_bot->Say("I can't find it.", LANGUAGE_UNIVERSAL);
             m_lootCurrent = 0;
             return;
         }

         if ( !go->isSpawned() ) {
             m_bot->Say("It is not there anymore.", LANGUAGE_UNIVERSAL);
             return;
         }
 ...
 else if (text == "survey")
 {
      ...
      if (result)
      {
         do
         {
			 ...
			 ObjectMgr *test = 0;
             GameObjectTemplate const * gInfo = test->GetGameObjectTemplate(entry);
...
void PlayerbotAI::SetQuestNeedItems()
{
    ...
    for(QuestStatusMap::iterator iter=m_bot->getQuestStatusMap().begin(); iter!=m_bot->getQuestStatusMap().end(); ++iter)
    {
        ...
        {
			if(!qInfo->ReqItemCount[i] || (qInfo->ReqItemCount[i]-qData->ItemCount[i]) <= 0) continue;
            m_needItemList[qInfo->ReqItemId[i]] = (qInfo->ReqItemCount[i]-qData->ItemCount[i]);
        }
    }
